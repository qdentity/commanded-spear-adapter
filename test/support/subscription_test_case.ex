defmodule Commanded.EventStore.Spear.SubscriptionTestCase do
  ...
  define_tests do
    ...
    setup %{test: test} do
      streams =
        Map.new(0..3, fn i ->
          stream_id = {i, test} |> :erlang.phash2() |> Integer.to_string()
          {String.to_atom("stream#{i}"), stream_id}
        end)
    ...
    describe "transient subscription to all streams" do
      test "should receive events appended to any stream", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 0, build_events(1))

        received_events = assert_receive_events(event_store, event_store_meta, count: 1, from: 1)
        assert Enum.map(received_events, & &1.stream_id) == [s.stream1]
        assert Enum.map(received_events, & &1.stream_version) == [1]

        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 0, build_events(2))

        received_events = assert_receive_events(event_store, event_store_meta, count: 2, from: 2)
        assert Enum.map(received_events, & &1.stream_id) == [s.stream2, s.stream2]
        assert Enum.map(received_events, & &1.stream_version) == [1, 2]

        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 0, build_events(3))

        received_events = assert_receive_events(event_store, event_store_meta, count: 3, from: 4)
        assert Enum.map(received_events, & &1.stream_id) == [s.stream3, s.stream3, s.stream3]
        assert Enum.map(received_events, & &1.stream_version) == [1, 2, 3]

        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 1, build_events(2))

        received_events = assert_receive_events(event_store, event_store_meta, count: 2, from: 7)
        assert Enum.map(received_events, & &1.stream_id) == [s.stream1, s.stream1]
        assert Enum.map(received_events, & &1.stream_version) == [2, 3]
    ...
    describe "persistent subscription to a single stream" do
      test "should receive `:subscribed` message once subscribed", %{
        ...
        {:ok, subscription} =
          event_store.subscribe_to(event_store_meta, s.stream1, "subscriber", self(), :origin, [])
      ...
      test "should receive events appended to stream", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 0, build_events(1))
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 1, build_events(2))
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 3, build_events(3))
      ...
      test "should skip existing events when subscribing from current position", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 3, build_events(3))
        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 0, build_events(3))
        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 0, build_events(3))
      ...
      test "should receive events already appended to stream", %{
        ...
        {:ok, subscription} =
          event_store.subscribe_to(event_store_meta, s.stream3, "subscriber", self(), :origin, [])

        assert_receive {:subscribed, ^subscription}

        assert_receive_events(event_store, event_store_meta, subscription, count: 3, from: 1)

        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 3, build_events(1))
        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 4, build_events(1))
    ...
    describe "persistent subscription concurrency" do
      ...
      test "should distribute events amongst subscribers", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 0, build_events(6))
      ...
      test "should distribute events to subscribers using optional partition by function", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream0, 0, build_events(2))
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 0, build_events(2))
        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 0, build_events(2))

        :ok = event_store.append_to_stream(event_store_meta, s.stream0, 2, build_events(2))
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 2, build_events(2))
        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 2, build_events(2))

        :ok = event_store.append_to_stream(event_store_meta, s.stream0, 4, build_events(1))
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 4, build_events(1))
        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 4, build_events(1))
      ...
      test "should exclude stopped subscriber from receiving events", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 0, build_events(2))
    ...
    describe "resume subscription" do
      test "should resume from checkpoint", %{
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream1, 0, build_events(1))
        :ok = event_store.append_to_stream(event_store_meta, s.stream2, 0, build_events(1))
        ...
        assert_receive {:events, ^subscriber1, received_events}
        assert length(received_events) == 1
        assert Enum.map(received_events, & &1.stream_id) == [s.stream2]
        ...
        :ok = event_store.append_to_stream(event_store_meta, s.stream3, 0, build_events(1))

        assert_receive {:events, ^subscriber2, received_events}
        assert length(received_events) == 1
        assert Enum.map(received_events, & &1.stream_id) == [s.stream3]
